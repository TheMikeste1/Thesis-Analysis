...
df_described = df_described[
    (df_described["coordination_mechanism"] == "Expert")
    # & (~df_described["shifted"])
]
...
facet.fig.suptitle("No Preference Change - Expert");
```
...
facet.fig.subplots_adjust(top=0.90)
facet.fig.suptitle("No Preference Change - Expert");
```

```python
df_described["improvement/mean/derivative"]
```
